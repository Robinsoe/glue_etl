...
class ETLFunctions(object):
    ...
    def create_glue_job(self, name, script_path, kargs=None):
        ...
        if name not in g_job_list:
            ...
            if kargs:
                defaultarguments = {
                    ...
                    '--enable-metrics': '',
                    '--job-language': 'python',
                    '--glue_db': kargs['glue_db'],
                    '--s3_bucket': kargs['s3_bucket'],
                    '--svr': kargs['svr'],
                    '--db': kargs['db'],
                    '--sch': kargs['sch'],
                    '--tbl': kargs['tbl'],
                    '--s3_bucket_etc': kargs['s3_bucket_etc'],
                    '--api': kargs['api'],
                }
                connections = {
                    'Connections': [
    ...
    def delete_source_to_glue_cat_etl(self, svr, db, sch, tbl):
    ...
    # delete workflows function
    def create_source_to_glue_cat_etl(self, svr, db, sch, tbl, type, group, con_list, s3_bucket_name):
        ...
        # start_wf(workflow_name)

    def create_glue_repartition_etl_all(self, svr, db, sch, tbl, type, group, api, s3_bucket_name, s3_bucket_etc, target_glue_db):
        ...
        script_path = f's3://{s3_bucket_name}/scripts/repartition_glue_script.py'
    ...
    def delete_glue_repartition_etl_all(self, svr, db, sch, tbl, target_glue_db):
